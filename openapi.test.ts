...
Deno.test("OpenAPI specification validation", async () => {
  ...
  }).openapi({ ref: 'User' }); // Corrected: Use { ref: 'User' }

  const CreateUserSchema = z.object({
    name: z.string().openapi({ example: "Jane Doe" }),
  }).openapi({ ref: 'CreateUser' }); // Corrected: Use { ref: 'CreateUser' }
    

  // Example route
  api.get(
    "/users/:id",
    z.object({ id: z.coerce.number().openapi({example: 123}) }).openapi({ ref: 'GetUserParams' }), // Corrected: Use { ref: 'GetUserParams' }
    UserSchema,
    "Get a user by ID",
  ...
  Deno.test("Validate /users/:id GET endpoint", () => {
    ...
    if (idParam.name !== "id" || idParam.in !== "path" || idParam.schema.type !== "integer") {
      throw new Error("Invalid 'id' parameter definition for '/users/{id}' GET");
    }
    ...
    if(response200.content['application/json'].schema.$ref !== '#/components/schemas/User'){
      throw new Error("Response schema for '/users/{id}' GET is incorrect");
    }
  });
  ...
  Deno.test("Validate /users POST endpoint", () => {
    ...
    if(requestBody.content['application/json'].schema.$ref !== '#/components/schemas/CreateUser') {
      throw new Error("Request body schema for '/users' POST is incorrect");
    }
  ...
  Deno.test("Validate User schema", () => {
    ...
    if(!userSchema.properties.id || userSchema.properties.id.type !== 'integer'){
      throw new Error("'User' schema should have an 'id' property of type 'integer'");
    }
    
    if(!userSchema.properties.name || userSchema.properties.name.type !== 'string'){
      throw new Error("'User' schema should have a 'name' property of type 'string'");
    }
  });
  ...
  Deno.test("Validate CreateUser schema", () => {
    ...
    if(!createUserSchema.properties.name || createUserSchema.properties.name.type !== 'string'){
      throw new Error("'CreateUser' schema should have a 'name' property of type 'string'");
    }
  });
   ...
   Deno.test("External OpenAPI Schema Validation (Sanity Check)", async () => {
    try {
      ...
      console.log("OpenAPI spec is valid (validated with @apidevtools/swagger-parser)");
    } catch (err) {
