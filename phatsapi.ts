...
>(
    ...
    let request;
    try {
        request = await requestSchema.parseAsync({
            ...params,
            ...query,
            ...body,
        });
...
export class PhatsAPI {
    ...
    >(
        ...
        this.hono.get(path, describeRoute(routeConfig), async (c) =>
            handleRequest(c, requestSchema, responseSchema, handler)
        );
    }
    ...
    >(
        ...
        this.hono.post(path, describeRoute(routeConfig), async (c) =>
            handleRequest(c, requestSchema, responseSchema, handler)
        );
    }
    ...
    >(
        ...
        this.hono.put(path, describeRoute(routeConfig), async (c) =>
            handleRequest(c, requestSchema, responseSchema, handler)
        );
    }
    ...
    >(
        ...
        this.hono.delete(path, describeRoute(routeConfig), async (c) =>
            handleRequest(c, requestSchema, responseSchema, handler)
        );
    }

    /**
     * Handles fetch requests for the application.
     *
    ...
    /**
     * Adds Hono middleware to the application.
     *
     * @param args - Arguments passed to the Hono use method.
     */
